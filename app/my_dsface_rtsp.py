...
def main(input_path, mount_point, port=8554):
    ...
    queue_postosd = Gst.ElementFactory.make('queue', 'queue_postosd')
    ...
    required = [source, nvvidconvsrc, caps_vidconvsrc, streammux, pgie, tracker, sgie1,
                nvvidconv, nvosd, nvvidconv_postosd, queue_postosd, caps, encoder, queue_encoder, h264parse, rtppay, udpsink]
    ...
    try:
        ...
        encoder.set_property('bitrate', 4000000 if encoder_factory == 'nvv4l2h264enc' else 8000)
    except Exception:
        pass

    # caps for encoder input
    if encoder_factory == 'x264enc':
        caps.set_property('caps', Gst.Caps.from_string('video/x-raw, format=I420'))
    else:
        caps.set_property('caps', Gst.Caps.from_string('video/x-raw(memory:NVMM), format=I420'))

    # configure rtph264pay and udpsink for local RTP streaming
    try:
    ...
    nvvidconv_postosd.link(queue_postosd)
    queue_postosd.link(caps)
    caps.link(encoder)
    ...
    # Set up RTSP server reading local RTP from the pipeline
    server = GstRtspServer.RTSPServer()
    mounts = server.get_mount_points()
    factory = GstRtspServer.RTSPMediaFactory()
    factory.set_launch('( udpsrc port=5000 caps="application/x-rtp,media=video,encoding-name=H264,payload=96" ! rtpjitterbuffer latency=100 ! rtph264depay ! h264parse ! rtph264pay name=pay0 pt=96 )')
...
def parse_args():
    parser = argparse.ArgumentParser(description='DeepStream file input to RTSP server streaming H264')
    ...
    return input_path, args.mount, args.port


if __name__ == '__main__':
    input_path, mount_point, port = parse_args()
    script_dir = os.path.dirname(os.path.realpath(__file__))
    load_face_embeddings(os.path.join(script_dir, 'models/embeddings.npz'),
                         os.path.join(script_dir, 'models/names.txt'))
    sys.exit(main(input_path, mount_point, port))
